    ...
    function T(a) {
      ...
      function c(e) {
        ...
        for (a = 0; a < e.length; ++a) {
          h = e[a], h[0] <= g[1] + 1 ? g[1] = Math.max(g[1], h[1]) : c.push(g = h);
        }

        for (a = 0; a < c.length; ++a) {
          h = c[a], b.push(f(h[0])), h[1] > h[0] && (h[1] + 1 > h[0] && b.push("-"), b.push(f(h[1])));
        }
      ...
      function m(e) {
        ...
        if (e.ignoreCase && q) for (g = 0; g < b; ++g) {
          k = a[g], e = k.charAt(0), 2 <= k.length && "[" === e ? a[g] = c(k) : "\\" !== e && (a[g] = k.replace(/[a-zA-Z]/g, function (a) {
            a = a.charCodeAt(0);
      ...
      function f(a) {
        for (var l = a.i, n = a.h, b = [l, "pln"], p = 0, q = a.a.match(m) || [], r = {}, e = 0, t = q.length; e < t; ++e) {
          var z = q[e],
              ...
              g = void 0,
              h;
          if ("string" === typeof v) h = !1;else {
            var k = c[z.charAt(0)];
            if (k) g = z.match(k[1]), v = k[0];else {
              for (h = 0; h < E; ++h) {
    ...
    function L(a, d, f) {
      function c(a) {
        ...
        if (1 == b && !t.test(a.className)) {
          if ("br" === a.nodeName.toLowerCase()) m(a), a.parentNode && a.parentNode.removeChild(a);else for (a = a.firstChild; a; a = a.nextSibling) {
            c(a);
          }
        } else if ((3 == b || 4 == b) && f) {
          var e = a.nodeValue,
              d = e.match(q);
          d && (b = e.substring(0, d.index), a.nodeValue = b, (e = e.substring(d.index + d[0].length)) && a.parentNode.insertBefore(l.createTextNode(e), a.nextSibling), m(a), b || a.parentNode.removeChild(a));
        }
      }
      ...
      function m(a) {
        function c(a, b) {
          ...
          if (k) {
            ...
            for (var f = d; f; f = d) {
              d = f.nextSibling, k.appendChild(f);
            }
          }
      ...
      for (var p = 0, r = b.length; p < r; ++p) {
        n = b[p], n.className = "L" + (p + d) % 10, n.firstChild || n.appendChild(l.createTextNode("\xA0")), w.appendChild(n);
      }
          ...
          for (var c = D.PR_SHOULD_USE_CONTINUATION ? b.now() + 250 : Infinity; p < x.length && b.now() < c; p++) {
            ...
            if ((l !== g || r.test(n)) && !e.test(n)) {
              ...
              if (!m) {
                ...
                if (B.test(d.tagName)) u = 1;else var u = d.currentStyle,
                    y = t.defaultView,
...
var O2Validator = /*#__PURE__*/function () {
  function O2Validator(formInstance) {
    _classCallCheck(this, O2Validator);

    this.formInstance = formInstance;
    this.initCallbacks();
  }

  _createClass(O2Validator, [{
    key: "validate",
    value: function validate() {
      this.errors = [];
      var $formFields = $(this.formInstance).find('._field'),
          hasErrors = false,
          self = this;
    ...
    value: function initCallbacks() {
      this.callbacks = {
        ...
        checked: function checked(field) {
          var $input = $(field).find('input');
          var checker = false;
          $input.each(function () {
    ...
    key: "setMessage",
    value: function setMessage(field, msg) {
      $(field).find('._error-msg').html(msg);
    }
  }, {
    key: "showGlobalErrors",
    value: function showGlobalErrors() {
      var errorsHtml = this.errors.join('</li><li>');
      var $errorsBlock = $(this.formInstance).find('._global-errors');
      $errorsBlock.html("<ul><li>".concat(errorsHtml, "</li></ul>"));
      if (!this.errors.length) $errorsBlock.removeClass('show');else $errorsBlock.addClass('show');
    }
  }, {
    key: "setErrors",
    value: function setErrors(errors) {
      for (var fieldCode in errors) {
        var $field = $(this.formInstance).find("._field[data-code=\"".concat(fieldCode, "\"]"));
        $field.addClass('error');
...
o2.gInputFile = {
  fileSelection: function fileSelection() {
    $(".g-input-file__file-inp").on('change', function (event) {
  ...
  close: function close(instance) {
    var touch = $(instance).parents('.g-input-file__img');
    touch.remove();
  }
};
"use strict";
...
o2.gPopup = {
  outListener: false,
  open: function open(contentClass) {
    this.removeListner();
    var popupHtml = $(".".concat(contentClass)).html();
    ...
    var $popup = $('._overlay').find('._popup-content');
    var self = this;
    setTimeout(function () {
      self.outListener = o2.clickOutside($popup, function () {
        self.close();
      });
    }, 10);
  },
  ...
  removeListner: function removeListner() {
    if (this.outListener) document.removeEventListener('click', this.outListener);
    this.outListener = false;
  },
  escEvent: false,
  setEscEvent: function setEscEvent() {
...
o2.gRangeSlider = {
  init: function init() {
    ...
    $('._range-slider').each(function (index, element) {
      var dataParams = $(element).data();
      var start = $(element).data('start');
      var connect = [true, false];
      ...
      slider.on('set', _this.set);
      slider.on('update', _this.update);
    });
  },
  set: function set(values, handle) {
    if (handle == 0) $(this.target).siblings('._min').trigger('change');else $(this.target).siblings('._max').trigger('change');
  },
  update: function update(values) {
    var $slider = $(this.target).parents('._range-slider-wrap');
    var firstValue = o2.gRangeSlider.format.from(values[0].toString());
    $slider.find('._min').val(firstValue);
    $slider.find('._label-to').html(values[0]);
    var from = o2.gRangeSlider.format.to(this.options.range.min);
    $slider.find('._label-from').html(from);

    if (values.length > 1) {
      var secondVal = o2.gRangeSlider.format.from(values[1].toString());
  ...
  format: {
    ...
    from: function from(stringNumber) {
...
o2.gSelect = {
  ...
  open: function open($select) {
    ...
    var listner = o2.clickOutside($select, function () {
      _this.close();
    });
  ...
  close: function close() {
    ...
    var _iterator = _createForOfIteratorHelper(this.outListeners),
        _step;
  ...
  toggle: function toggle(instance) {
    ...
    if (!$select.hasClass('g-select--opened')) this.open($select);else this.close();
  },
   ...
   */
  setName: function setName($select, name) {
   ...
   */
  setSelectedValue: function setSelectedValue($select, selectedValue) {
   ...
   */
  selecttItem: function selecttItem(instance) {
    var $select = $(instance).parents('._select');
...
o2.gSpoiler = {
  spoilerToggle: function spoilerToggle(title) {
  ...
  defaultSpoiler: function defaultSpoiler(title) {
    $(title).toggleClass('g-spoiler__title--active').siblings('._g-spoiler__list').slideToggle(300);
  },
  oneShowSpoiler: function oneShowSpoiler(title) {
    var $title = $(title);
    var $gSpoiler = $title.parents('._g-spoiler');
    var isActive = $title.hasClass('g-spoiler__title--active');
  ...
  onlyOneSpoiler: function onlyOneSpoiler(title) {
    var $title = $(title);
    var $gSpoiler = $title.parents('._g-spoiler');
    var isActive = $title.hasClass('g-spoiler__title--active');
    $gSpoiler.find('._g-spoiler__list').slideUp(300);
...
o2.gTabs = {
  open: function open(instance, tabId) {
    ...
    var $tabsContainer = $(instance).closest('._tabs-container');
...
o2.authForm = {
  submit: function submit(instance) {
    var validator = new O2Validator(instance);
    validator.callbacks.custom = this.customValidation;
    if (!validator.validate()) return false; // отправили запрос и якобы получили ошибку
  ...
  customValidation: function customValidation(field) {
...
o2.uiKit = {
  ...
  components: [{
    ...
    name: 'Range slider',
    "class": '._range-slider-component'
  }, {
    name: 'Input',
    "class": '._input-component'
  }, {
    name: 'Input width icon',
    "class": '._input-width-icon-component'
  }, {
    name: 'Input masked',
    "class": '._input-masked-component'
  }, {
    name: 'Checkbox',
    "class": '._checkbox-component'
  }, {
    name: 'Radio',
    "class": '._radio-component'
  }, {
    name: 'Tabs',
    "class": '._tabs-component'
  }, {
    name: 'Date picker',
    "class": '._date-picker-component'
  }, {
    name: 'Adaptive',
    "class": '._adaptive-component'
  }, {
    name: 'Slider',
    "class": '._slider-component'
  }, {
    name: 'Button',
    "class": '._button-component'
  }, {
    name: 'Валидация форм',
    "class": '._validation-component'
  }, {
    name: 'Popup',
    "class": '._popup-component'
  }, {
    name: 'Pagination',
    "class": '._pagination-component'
  }, {
    name: 'Notifications',
    "class": '._notifications-component'
  }, {
    name: 'Input file',
    "class": '._input-file_component'
  }, {
    name: 'Yamap',
    "class": '._yamap-component'
  }, {
    name: 'Spoiler',
    "class": '._spoiler-component'
  }],
  init: function init() {
    PR.prettyPrint();
  ...
  getComponents: function getComponents() {
    ...
    return this.components.filter(function (component) {
      return component.name.toLowerCase().indexOf(_this.componentsFilter) !== -1;
    });
  },
  renderComponentsList: function renderComponentsList() {
    var componentsList = document.querySelector('._components-list');
    var comopsHtml = '';

    var _iterator = _createForOfIteratorHelper(this.getComponents()),
        _step;

    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        var component = _step.value;
        comopsHtml += "<li onclick=\"o2.uiKit.selectComponent(this,'".concat(component["class"], "')\">").concat(component.name, "</li>");
      }
    } catch (err) {
      _iterator.e(err);
    } finally {
  ...
  selectComponent: function selectComponent(instance, className) {
    ...
    var _iterator3 = _createForOfIteratorHelper(document.querySelectorAll('.ui-kit__components-list .active')),
        _step3;
    ...
    try {
      for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
        ...
        document.querySelector(_component["class"]).style.display = 'none';
      }
    } catch (err) {
      _iterator4.e(err);
    } finally {
  ...
  filterComponents: function filterComponents(instance) {
